...
export default function Footer({ visible }: { visible?: boolean }) {
  ...
  return (
    <footer
      className={`${pathname === paths.toWheel && !visible ? "hidden md:block" : "block md:hidden"} ${pathname === paths.toHome && "block"}  w-screen animate-fade-up text-center text-[0.65rem] tracking-tight opacity-0 md:text-[0.9rem]`}
